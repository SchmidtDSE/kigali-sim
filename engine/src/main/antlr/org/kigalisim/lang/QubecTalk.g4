...
unitOrRatio: unit ((DIV_ | EACH_) unit)*;
...
expression: number  # simpleExpression
  ...
  | GET_ target=stream INTEGER_ (YEAR_ | YEARS_) AGO_  # getStreamPrior
  | GET_ target=stream INTEGER_ (YEAR_ | YEARS_) AGO_ AS_ conversion=unitOrRatio  # getStreamPriorConversion
  | GET_ target=stream INTEGER_ (YEAR_ | YEARS_) AGO_ OF_ rescope=string  # getStreamIndirectPrior
  | GET_ target=stream INTEGER_ (YEAR_ | YEARS_) AGO_ OF_ rescope=string AS_ conversion=unitOrRatio  # getStreamIndirectPriorConversion
...
equalsStatement: CONSUME_ value=unitValue  # equalsAllYears
  | CONSUME_ value=unitValue duration=during  # equalsDuration
  ;
