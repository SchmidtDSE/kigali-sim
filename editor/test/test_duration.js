...
function buildDurationTests() {
  QUnit.module("ParsedYear", function () {
    ...
    QUnit.module("fromLegacyValue", function () {
      QUnit.test("handles numeric primitive values", function (assert) {
        const parsedYear = ParsedYear.fromLegacyValue(2025);
        assert.strictEqual(parsedYear.getNumericYear(), 2025);
        assert.strictEqual(parsedYear.getYearStr(), "2025");
      });

      QUnit.test("handles string primitive values", function (assert) {
        const parsedYear = ParsedYear.fromLegacyValue("beginning");
        assert.strictEqual(parsedYear.hasFiniteNumericYear(), false);
        assert.strictEqual(parsedYear.getYearStr(), "beginning");
      });

      QUnit.test("handles null values", function (assert) {
        const parsedYear = ParsedYear.fromLegacyValue(null);
        assert.strictEqual(parsedYear.hasFiniteNumericYear(), false);
        assert.strictEqual(parsedYear.getYearStr(), "");
      });

      QUnit.test("handles undefined values", function (assert) {
        const parsedYear = ParsedYear.fromLegacyValue(undefined);
        assert.strictEqual(parsedYear.hasFiniteNumericYear(), false);
        assert.strictEqual(parsedYear.getYearStr(), "");
      });

      QUnit.test("handles object from visitNumber with positive value", function (assert) {
        const legacyValue = {value: 2025, originalString: "2025"};
        const parsedYear = ParsedYear.fromLegacyValue(legacyValue);
        assert.strictEqual(parsedYear.getNumericYear(), 2025);
        assert.strictEqual(parsedYear.getYearStr(), "2025");
      });

      QUnit.test("handles object from visitNumber with negative value", function (assert) {
        const legacyValue = {value: -5, originalString: "-5"};
        const parsedYear = ParsedYear.fromLegacyValue(legacyValue);
        assert.strictEqual(parsedYear.getNumericYear(), -5);
        assert.strictEqual(parsedYear.getYearStr(), "-5");
      });
      ...
      QUnit.test(
        "handles object from visitNumber with formatted original string",
        function (assert) {
          const legacyValue = {value: 2025, originalString: "2,025"};
          const parsedYear = ParsedYear.fromLegacyValue(legacyValue);
          assert.strictEqual(parsedYear.getNumericYear(), 2025);
          assert.strictEqual(parsedYear.getYearStr(), "2,025");
        },
      ...
      QUnit.test("handles object from visitNumber with zero value", function (assert) {
        const legacyValue = {value: 0, originalString: "0"};
        const parsedYear = ParsedYear.fromLegacyValue(legacyValue);
        assert.strictEqual(parsedYear.getNumericYear(), 0);
        assert.strictEqual(parsedYear.getYearStr(), "0");
      });

      QUnit.test("handles string inputs from UI forms", function (assert) {
        const parsedYear = ParsedYear.fromLegacyValue("2025");
        assert.strictEqual(parsedYear.hasFiniteNumericYear(), false);
        assert.strictEqual(parsedYear.getYearStr(), "2025");
      });
      ...
      QUnit.test("handles NaN values", function (assert) {
        const parsedYear = ParsedYear.fromLegacyValue(NaN);
        assert.strictEqual(parsedYear.hasFiniteNumericYear(), false);
        assert.strictEqual(parsedYear.getYearStr(), "NaN");
      });

      QUnit.test("handles Infinity values", function (assert) {
        const parsedYear = ParsedYear.fromLegacyValue(Infinity);
        assert.strictEqual(parsedYear.hasFiniteNumericYear(), false);
        assert.strictEqual(parsedYear.getYearStr(), "Infinity");
      });

      QUnit.test("handles object with NaN value", function (assert) {
        const legacyValue = {value: NaN, originalString: "invalid"};
        const parsedYear = ParsedYear.fromLegacyValue(legacyValue);
        assert.strictEqual(parsedYear.hasFiniteNumericYear(), false);
        assert.strictEqual(parsedYear.getYearStr(), "invalid");
      });
    });
  });
