  ...
  QUnit.module("UiTranslatorCompiler", function () {
    ...
    buildTest("converts policy with recharge command", "/examples/ui/policy_recharge_test.qta", [
      ...
      (result, assert) => {
        ...
        assert.equal(recharge.getPopulationEngineNumber().getValue(), 15, "Recharge population should be 15");
    ...
    buildTest("updateMetadata round-trips weibull retirement", "/examples/ui/bau_single.qta", [
      (result, assert) => {
        assert.ok(result.getIsCompatible());
        ...
        const newMetadata = new SubstanceMetadata(
          ...
          "", // initialChargeImport
          "", // initialChargeExport
          "20 year old mean weibull", // retirement
        );

        substance.updateMetadata(newMetadata, "app1");

        // Verify retirement parses as a Weibull retire without replacement
        const retire = substance.getRetire();
        assert.ok(retire !== null);
        ...
        // Verify codegen re-emits the Weibull form
        const code = substance.toCode(0);
        assert.ok(code.indexOf("retire 20 year old mean weibull") !== -1);
        assert.ok(code.indexOf("with replacement") === -1);
      },
    ]);

    buildTest("updateMetadata round-trips exact retirement", "/examples/ui/bau_single.qta", [
      (result, assert) => {
        assert.ok(result.getIsCompatible());
        ...
        const newMetadata = new SubstanceMetadata(
          ...
          "", // initialChargeImport
          "", // initialChargeExport
          "5 year old exact", // retirement
        );

        substance.updateMetadata(newMetadata, "app1");

        // Verify retirement parses as an exact retire without replacement
        const retire = substance.getRetire();
        assert.ok(retire !== null);
        ...
        // Verify codegen re-emits the exact form
        const code = substance.toCode(0);
        assert.ok(code.indexOf("retire 5 year old exact") !== -1);
        assert.ok(code.indexOf("with replacement") === -1);
      },
    ]);
