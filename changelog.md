...
In specific circumstances, using `replace` on a stream tracked in equipment units (for example via `set import to 100 units`) could cause that stream to lose its unit-tracking basis and quietly fall back to mass-based (kg) tracking instead. This required a fairly uncommon combination: a `replace` policy followed, in the same year, by a `cap` or `floor ... displacing` policy on the same substance, together with recharge or precharge (servicing) configured. In that specific case, the affected cap or floor could end up constraining far more than intended relative to business as usual, even when the cap or floor would not have bound on its own. While fixing this, we also consolidated the bookkeeping logic shared by `replace` and `cap`/`floor ... displacing` so this class of issue cannot resurface through either path. Tests have been added to prevent regression. We apologize for this issue. See [#843](https://github.com/SchmidtDSE/kigali-sim/pull/843) and [#845](https://github.com/SchmidtDSE/kigali-sim/pull/845).
...
### Bare sales percentage using populations

**Status**: Preview released August 4, 2026. Full release August 5, 2026.

**Classification**: Bug

A percent cap or floor without specifying `% current` or `% prior year` was causing total population to be used as a base rather than the stream's own last specified value. This was only encountered when the affected stream (`domestic`, `import`, `sales`) was tracked in equipment units (for example via `set domestic to 1000 units`) rather than mass-based tracking (`mt` or `kg`), and often led the operation to have far less effect than intended. Floor is less commonly used than cap but was equally affected. Tests have been added to prevent regression. We apologize for this issue.

### Recharge excluded from unit-based cap/floor basis
