...
In addition to official participation in international policy-relevant treaty meetings [@oweg; @excom; @pacificnetwork; @seasianetwork] and extended co-design with over a dozen nations and supporting organizations with news media coverage [@nbc], multiple governments publicly acknowledge use of the software [@credits].
...
## Simulation engine
Some countries and supporting organizations work from trade records and others from industry census or observed emissions. As opposed to a unidirectional structure assuming the location of observation within systems, the engine propagates user-provided values through substance flows and lifecycles to estimate the unmeasured quantities. It then layers complex policy interventions such as permitting and recycling on top of the "business as usual" scenario.
...
# Implementation
Migrated from an original JavaScript implementation for performance, Kigali Sim runs browser-based via WASM or via JVM [@teavm]. Without transmitting simulations to external servers, both enable privacy-preserving zero-installation parallel local computation with BigDecimal [@bigdecimal]. ANTLR [@antlr] parses QubecTalk code in both Java and JavaScript^[A JS-based ANTLR visitor facilitating real-time translation between the UI-editor and the code-based editor.].
